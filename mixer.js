...
import {dbValues, delay, hexToByteArray, levelMatrixInputs, levelMatrixOutputs, muteParameters, panMatrix, panValues} from './helpers.js';
  ...
  constructor(instanceContext, ip, port, autoReconnect) {
    ...
    parseIncomingMessages(hexMessage) {
      ...
      while (i < byteArray.length) {
        ...
        // If no known pattern found, log the unknown part and move forward
          ...
          console.info(
              'Could not parse message - unknown format. Skipping one byte and trying again.')
      ...
      // Handle Parameter Status Messages (Mute, Volume, Pan)
        ...
        // Handle Volume Packets - Inputs and FX to Outputs and FX
          ...
          if (parameter) {
            ...
            const nearestLabel =
                this.findNearestDBLabel(valueCoarse, valueFine);
        ...
        // Handle Volume Packets - Outputs, FX unit inputs, and DCAs
          ...
          if (parameter) {
            ...
            if (nearestLabel) {
              ...
              // Log and update feedback for the volume control
              console.log(
                  `Received level for ${parameter.label}:`, nearestLabel.label,
                  'dB');
            } else {
        ...
        // Handle Pan Packets (MSB: 0x50 to 0x56)
          ...
          if (parameter) {
            ...
            const nearestLabel =
                this.findNearestPanLabel(valueCoarse, valueFine);
            ...
            if (nearestLabel && target) {
              ...
              console.log(
                  `Pan level for ${parameter.label} ${target}:`,
                  nearestLabel.label);
        ...
        console.log(
            ...
            byteArray.map(byte => byte.toString(16).padStart(2, '0'))
                .join(' '));
      }
    }
    ...
    setAbsValue(msb, lsb, vc, vf) {
      const message = [
        ...
        parseInt(lsb, 16), 0xB0, 0x06, parseInt(vc, 16), 0xB0, 0x26,
        parseInt(vf, 16)
      ];
      this.sendMIDIMessage(message);
    ...
    controlSoftKey(key, state) {
      const midiMessage = [
        (state == 'press' ? 0x90 : 0x80), parseInt(key, 16),
        (state == 'press' ? 0x7F : 0x00)
      ];
      this.sendMIDIMessage(midiMessage)
    }
